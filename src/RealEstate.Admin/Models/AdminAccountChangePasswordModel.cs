...
public class AdminAccountChangePasswordModel
{
...
public class AdminAccountChangePasswordModelValidator : AbstractValidator<AdminAccountChangePasswordModel>
{
    private readonly IStringLocalizer<GlobalStrings> _localizer;
    
    public AdminAccountChangePasswordModelValidator(IStringLocalizer<GlobalStrings> localizer)
    {
        _localizer = localizer;
        
        RuleFor(_ => _.CurrentPassword)
            .NotEmpty().WithMessage($"'{_localizer["Current Password"]}' {_localizer["is empty"]}.")
            .MinimumLength(6).WithMessage($"'{_localizer["Current Password"]}' {_localizer["minimum length must be"]} 6.")
            .MaximumLength(16).WithMessage($"'{_localizer["Current Password"]}' {_localizer["maximum length must be"]} 16.");
